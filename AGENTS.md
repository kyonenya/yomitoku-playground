...
- グローバル側の恒久対策として `C:\Users\kyone\.codex\config.toml` の `[windows] sandbox = "unelevated"` へ変更済み。新しい `codex sandbox powershell -NoProfile -Command Get-Date` は成功。
- このワークスペースでは Codex のプロジェクトローカル設定 `.codex/config.toml` で `sandbox_mode = "danger-full-access"` にしている。`uv` cache や Python tempfile が AppData/Temp を使うため、Windows sandbox 内で動かすとアクセス拒否が出やすい。
...
- **scan 直下が単一の uv プロジェクト**（`pyproject.toml` + `uv.lock`）。
  ...
  - `.venv` は `uv sync` で生成。スクリプトは `uv run yomitoku.py ...` で実行する。
- **開発用チェックツール導入済み**（2026-06-07）。
  - linter: `uv run ruff check .`
  - formatter: `uv run ruff format .`（差分確認は `uv run ruff format . --diff`、確認のみは `uv run ruff format . --check`）
  - typechecker: `uv run ty check`
  - dev 依存は `pyproject.toml` の `[dependency-groups].dev` で管理。設定はほぼデフォルトで、ruff lint は一行 `if` 用に `E701` だけ ignore している。ただし `ruff format` は一行 `if` を複数行へ整形する。
...
- **ocrmypdf は別 venv に隔離**: `uv tool install ocrmypdf --python 3.11`。
  ...
  - Ghostscript・Tesseract は実行時に PATH から解決（現状維持）。
- **スクリプトの外部ツール解決を修正**: `yomitoku.py` の個人環境固有パスを廃止。
  - yomitoku: `uv run` 時に `.venv\Scripts` の `yomitoku` を解決。
  - jbig2: 環境変数による上書きや PATH 解決は廃止。プロジェクトルート直下の `jbig2enc-0.31-Windows-X64-MSVC\bin\jbig2.exe` を固定参照する。
...
## 定型化スクリプト `yomitoku.py`

YomiToku PDF生成、半分解像度JBIG2背景差し替え、PDF結合をまとめた定型スクリプトを作成した。

- 場所:
  - `yomitoku.py`（プロジェクト直下）
- 基本実行例（uv 経由）:
  - `uv run yomitoku.py "10_Scan\<本のフォルダ>\out"`
  - scan ディレクトリで実行する。`uv run` が `.venv`（yomitoku/torch/各ライブラリ）を自動で使う。
- `--output` を省略した場合:
  - 最終PDFは実行ディレクトリ直下の `output.pdf` に出る。
- `--workdir` を省略した場合:
  - YomiToku OCR 結果 PDF などの作業ファイルは `input_dir` の親に出る。
- `--workdir` を指定すると作業ファイルの出力先を変更できる。
...
3. YomiTokuをフォルダ指定で1回だけ起動する。
   - `yomitoku.exe input_dir -f pdf --pdf_quality high --outdir workdir`
...
## `yomitoku.py` の汎用化とシェルラッパー分離（2026-06-06）

`yomitoku.py` に混在していた「私のディレクトリ構成依存の処理」と「普遍的な処理」を分離した。

- `yomitoku.py` は**汎用ツール**にした。単独でも呼べる。
  - 引数は `input_dir`（`*.tif` が直接入ったフォルダ）、`--output` / `-o`（最終PDFパス、省略時は実行ディレクトリ直下の `output.pdf`）、`--workdir` / `-w`（YomiToku OCR 結果 PDF などの作業ファイル出力先、省略時は `input_dir` の親）、`--half`（二値化前に半分解像度へ縮小、既定は元解像度）。
  - `/out` を付ける・`/cache` を消す・出力名をフォルダ名にする・出力先を決める、といった構成依存は**持たない**。
  - 実行例: `uv run yomitoku.py "<入力フォルダ>" --output "本.pdf" --workdir "<作業ディレクトリ>" --half`
  - 半分解像度化を `yomitoku.py --half` に統合した（`make_otsu_tif` が `half=True` のとき LANCZOS で半分に縮小してから Otsu 二値化）。背景画素を半分にしてもページ物理サイズは元 TIFF の DPI と YomiToku PDF の MediaBox 基準で決まるため変わらない。
- 構成依存の処理は**薄いラッパー `yomitoku.ps1`** に切り出した（PowerShell で実行）。
  - `<本のフォルダ>\out` を TIFF フォルダとして `yomitoku.py` に渡す。
  - `<本のフォルダ>\out\cache` があれば消す。
  - 最終PDFを `<本のフォルダ>\yomitoku\<本のフォルダ名>.pdf`、作業ディレクトリを `<本のフォルダ>\yomitoku\yomitoku_pdf` にする。
  - `-Half` スイッチを受け取り、付いていれば `--half` を `yomitoku.py` にパススルーする（既定 OFF）。
  - 結果として最終PDFは従来どおり `<本のフォルダ>\yomitoku\<本のフォルダ名>.pdf` に出る。
  - 実行例: `.\yomitoku.ps1 "10_Scan\250620_Seminaire 1-1"` / `.\yomitoku.ps1 "..." -Half`
...
- `yomitoku.py` は既存の半分TIFFを再利用するが、JBIG2差し替え済み単ページPDFは毎回再生成する。
